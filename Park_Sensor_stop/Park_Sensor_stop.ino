...
int entering = 200; //light bar starts adding in purple
int approaching = 150; //light bar starts adding in green
int safedistance = 100; //light bar starts adding in yellow
int normalstop = 50; //light bar starts adding in red
int emergencystop = 7 ; //flashes warning lights - wanted value +3

int wait = 10*1000;  //#of seconds to wait before turning off 
                     //(multiplied by 1000 to convert to milliseconds)
float gap = 2.5;  //noise threshold for detecting movement


Adafruit_NeoPixel strip = Adafruit_NeoPixel(N_LEDS, PIN, NEO_GRB + NEO_KHZ800);
uint32_t white = strip.Color(127, 127, 127); //white
uint32_t off = strip.Color(0, 0, 0);    //off
uint32_t bG = strip.Color(8, 127, 100); //blue-green
uint32_t purple = strip.Color(100, 5, 80); //purple 255, 15, 200
uint32_t red = strip.Color(150, 0, 0);  //red
uint32_t green = strip.Color(0, 127, 0);  //green
uint32_t blue = strip.Color(0, 0, 127);  //blue
uint32_t yellow = strip.Color(90, 100, 0);
...
void colorChange2() {
  ...
  } else if(distancecm < (emergencystop-3+(val/10))){
    for (int i = 0; i < N_LEDS; i++) {
